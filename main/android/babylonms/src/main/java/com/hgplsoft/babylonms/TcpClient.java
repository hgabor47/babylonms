...
public class TcpClient {
    public static final String ID = "BMSTCPC";
    //public static final String SERVER_IP = "192.168.1.102"; //server IP address  Home Intra
    public String SERVER_IP = "192.168.42.100"; //server IP address  Android USB
    ...
    public void Start() {
        ...
        try {
            ...
            try {
                Log.e(ID, "TCP Connected");
                mBufferIn = socket.getInputStream();
                mBufferOut = new PrintWriter(socket.getOutputStream(), true);
                int charsRead = 0;
                byte[] buffer = new byte[4096]; //choose your buffer size if you need other than 1024
                while (mRun) {
                ...
                Log.e(ID, "TCP Response from server '");
            } catch (Exception e) {
                Log.e(ID, "TCP: Error 2", e);
            } finally {
                Log.e(ID, "TCP: Close ");
                try {
                    socket.close();
